...
If the message is valid and has not been previously seen, then it will send `IHAVE` with its message id attached to all mesh peers to tell them that it just receives a new message. If they want the full content of the message, they should send `IWANT` back.
