...
By selecting a high enough degree you can achieve a robust network with minimum latency. This allows messages to dispered resiliently across a network in a short amount of time.

The mesh that is selected is random and is constantly maintained via gossip control messages to ensure that quality of peers stays consistent and doesn't detoriate. Gossip Metadata is emitted during every heartbeat to allow peers who might have missed certain messages due to downstream message loss to be able to recover and retrieve them.
...
In announcesub, instead of sending the messages right away to mesh peers, you send `IHAVE` to them instead and see if they are interested in the message or not. If they are, they can send `IWANT`.

After sending `IWANT` to mesh peers, if there is no response after some timeout, you send `IWANT` to another peer that sent you `IHAVE` earliest. If you still get no response, keep going on.
...
| `timeout`            | Timeout for IWANT messages                        | 400 miliseconds    |
...
Peering state is how the router keeps track of the pubsub-capable peers its aware of and the relationship with each of them.
...
The `IHAVE` message is sent to mesh peers, when the local router receives a new message. It provides the remote peer with a single message id of the message that was recently seen by the local router.

### IWANT

The `IWANT` message requests the full content of a single message whose id was announced by a remote peer in an `IHAVE` message.
...
If the message is valid and has not been previously seen, then
- First, it will forward the message to every peer in `peers.floodsub[topic]` for backwards-compatibility with floodsub
- Second, it will forward the message to every gossipsub mesh peer for backwards-compatibility with gossipsub
- Finally, it will send `IHAVE` with its message id attached to all mesh peers to tell them that it just receives a new message. If they want the full content of the message, they should send `IWANT` back.
...
- Upon receiving `IWANT` after sending `IHAVE`, the router must send the full content back without an option to decline. Otherwise it will be deemed misbehaving.

- Upon receiving `IHAVE`, the router add the sending peer to a priority list of who sent `IHAVE` earliest for a particular message id. If it is the first `IHAVE` of a particular id, the router sends `IWANT` to that peer immediately.

  After the router sends `IWANT`, it will time out if it doesn't receive the message back in time, as indicated by `timeout`. If the timeout happens, the router will send `IWANT` to the next peer on the priority list. If it still times out, keep going with next peers until the list runs out of peers.

  Notice that timeouts can delay receiving the message. It's worth noting that timeouts are rare, since the peer just sent the router `IHAVE` so it means the network condition is already good and the peer should be able to send the actual message without problems.
...
- Second, publish the message to gossipsub peers with the logic defined in the [gossipsub spec][gossipsub-spec].
- Finally, publish the message to announcesub peers.
    - If the router is subscribed to the topic, it will send `IHAVE` to all mesh peers in `mesh[topic]` and reply with the actual message if it gets `IWANT` back.
...
The heartbeat serves three functions: [mesh maintenance](#mesh-maintenance) and [fanout maintenance](#fanout-maintenance).
...
The four control messages are `ControlIHave` for [`IHAVE`](#ihave) messages, `ControlIWant` for [`IWANT`](#iwant) messages, `ControlGraft` for [`GRAFT`](#graft) messages and `ControlPrune` for [`PRUNE`](#prune) messages.
...
- Let publishes just send the full content of messages to mesh peers, rather than `IHAVE`, because no one has really seen the message before. This saves one RTT, but it will kills anonymity so we are not sure yet to do it.
